  ...
  implements Comparable<Point>
{
   ...
   * point (x1, y1) if and only if either y0 < y1 or if y0 = y1 and x0 <
   * x1.
   *
   * @param that
   *        the other point
   * @return the value <tt>0</tt> if this point is equal to the argument
   *         point (x0 = x1 and y0 = y1); a negative integer if this
   *         point is less than the argument point; and a positive
   ...
   * defined to be +0.0 if the line segment connecting the two points is
   * horizontal; Double.POSITIVE_INFINITY if the line segment is
   * vertical; and Double.NEGATIVE_INFINITY if (x0, y0) and (x1, y1) are
   * equal.
   *
   * @param that
   *        the other point
   ...
   * provide for debugging; your program should not rely on the format
